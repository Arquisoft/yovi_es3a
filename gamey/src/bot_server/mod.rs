...
pub fn create_default_state() -> AppState {
    eprintln!("📦 Creando estado inicial con registro de bots...");
    let bots = create_all_bots(); // Construye todos los bots disponibles
    eprintln!("✓ Registro de bots creado exitosamente con {} bots", bots.count());
    AppState::new(bots)
}
...
pub async fn run_bot_server(port: u16) -> Result<(), GameYError> {
    eprintln!("🔧 Inicializando estado del servidor...");
    let state = create_default_state();

    eprintln!("🛣️ Creando router...");
    let app = create_router(state);

    let addr = format!("0.0.0.0:{}", port);

    eprintln!("🔌 Intentando enlazar puerto {}...", port);
    ...
    eprintln!("   - Status: http://{}/status", addr);
    eprintln!("   - Bots: http://{}/bots", addr);
